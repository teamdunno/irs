...
async fn handle_connection(
    ...
    'connection_handler: {
        ...
        let mut my_server_id = ServerId::try_from("000".to_owned()).unwrap();
        ...
        loop {
            tokio::select! {
                ...
                /*result = ts6_server_status.message_listener(&state, &mut message_receiver, &mut tcp_writer, &hostname) => {
                    match result {
                        Ok(_) => {},
                        Err(_) => {
                            break;
...
async fn tcp_listener(
    ...
    if !user_state.identified && user_state.is_populated() {
...
mod tests {
    ...
    async fn test_user_id_generator() {
        while userid_gen::increase_user_id().await.is_ok() {
            if userid_gen::get_user_id().await == ['A', 'B', 'C', 'D', 'E', 'F'] {
                userid_gen::manually_set_user_id(['Z', 'Z', 'Z', 'Z', 'Z', 'Y'].to_vec()).await;
                break;
            }

            dbg!(userid_gen::get_user_id().await);
        }

        while userid_gen::increase_user_id().await.is_ok() {
            if userid_gen::get_user_id().await == ['A', '1', '2', '3', '4', '5'] {
                // ff a bit
